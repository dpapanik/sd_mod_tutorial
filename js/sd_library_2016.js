var defaultTimeline = 144,
    ...
    groupCounter = 0,
    systemCounter = 0,
    stockColor = d3.rgb(0,215,255),
    flowColor = d3.rgb(0,215,255),
...
function utilities(timeline, periodic){
  return {
    ...
    pulseNormal: function (mean, sigma, i) { // Returns a normal distribution : function implemented from Wikipedia
      return  (1/(sigma * Math.sqrt(2*Math.PI) )) * Math.pow(Math.E, - Math.pow(i-(mean+0.5*sigma),2)/(2*Math.pow(sigma,2))); // NORMAL DISTRIBUTION EQUATION
    },
    pulseLogistic: function (mean, sigma, i) { // Returns a logistic distribution : function implemented from Wikipedia
          //area under curve using CDF:
          var CDF = function(m,s,x){return 1/(1+Math.pow(Math.E, -(x-m)/s))};
          var area = CDF(mean, sigma, timeline)-CDF(mean, sigma, 0);
          var rise = (1-area)/timeline; //this is how much I need to rise the curve so that the total area below equals 1
          return  rise + Math.pow(Math.E, -(i-mean)/sigma)/(sigma*Math.pow((1+Math.pow(Math.E,-(i-mean)/sigma)),2)); // LOGISTIC DISTRIBUTION EQUATION
